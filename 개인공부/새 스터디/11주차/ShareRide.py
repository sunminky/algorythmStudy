...
def djikstra(path, src, n) -> list:    #경로별 코스트 반환
    cost = [2000000 for _ in range(n+1)]
    queue = PriorityQueue()
    cost[src] = 0
...
def solution(n, s, a, b, fares):
    ...
    costs = []
    for fare in fares:
        path[int(fare[0])].append((int(fare[1]), int(fare[2])))
        path[int(fare[1])].append((int(fare[0]), int(fare[2])))

    costs.append(djikstra(path, s, n))
    costs.append(djikstra(path, a, n))
    costs.append(djikstra(path, b, n))
